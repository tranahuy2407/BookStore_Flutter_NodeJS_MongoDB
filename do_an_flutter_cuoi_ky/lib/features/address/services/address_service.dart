...
import 'package:do_an_flutter_cuoi_ky/models/user.dart';
...
class AddressServices {
  ...
  void placeOrder({
    ...
    try {
      ...
      httpErrorHandle(
        ...
        context: context,
        onSuccess: () {
          showSnackBar(context, 'Đơn hàng của bạn đã được đặt!');
          User user = userProvider.user.copyWith(
            cart: [],
          );
          userProvider.setUserFromModel(user);
          sendOrderNotificationToServer();
        },
      );
    } catch (e) {
      showSnackBar(context, e.toString());
  ...
  void sendOrderNotificationToServer() async {
    try {
      http.Response res = await http.post(
        Uri.parse('$uri/api/send-order-notification'),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: jsonEncode({
      ...
      print('Error sending order notification to server: $e');
    }
  }
}